...
const ProductCard: FC<ProductProps> = ({
	...
	return (
		...
		>
			...
			>
				<Image
					src={product.image_url == "" || product.image_url == null ? placeholderImage  : product.image_url}
					width={imgWidth}
					height={imgHeight}
					loading={imgLoading}
					quality={100}
			...
			>
				...
				>
					<span className="inline-block sm:text-base">₹{product.net_price}</span>
					<del className=" font-normal text-gray-800 text-sm">
						₹{product.sale_price}
					</del>
					{
						product.discount_percent > 0 ?
						<span className="inline-block text-sm">{product.discount_percent}% Off</span>
						:
						null
					}
				</div>
