...
import { removeFromCart, increaseQty, decreaseQty } from "../../Store/Action/Index"
	...
	return (
		...
		>
			<div className="relative flex w-24 md:w-28 h-24 md:h-28 rounded-md overflow-hidden bg-gray-200 flex-shrink-0 cursor-pointer me-4">
				...
				/> */}
				<img
					src={item.imageUrl ? item.imageUrl : "/assets/placeholder/cart-item.svg"}
					width={112}
					height={112}
					alt={item.name || "Product Image"}
					className="bg-gray-300 object-cover"
			...
			<div className="flex flex-col w-full overflow-hidden">
				...
				<div className="flex items-end justify-between">
					<Counter
						...
						onIncrement={() => dispatch(increaseQty(item.product_id))}
						onDecrement={() => dispatch(decreaseQty(item.product_id))}
						variant="dark"
					/>
